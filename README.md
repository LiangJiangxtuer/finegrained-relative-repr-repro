...
当前仓库已经完成 PAL 核心、COCO2014 全量 token extraction、K=512 主模型训练、COCO/Flickr30k retrieval 评测以及 5 个分类数据集 zero-shot 评测。最新进度、论文指标差距和后续复现步骤见 `REPRODUCTION_SUMMARY.md` 与 `docs/results_snapshot.md`。

仍待完成的 paper-grade 部分包括 VOC20/Context/ADE20K segmentation 全量评测、K / `tau_p` / token-CAP 消融、官方 split / CKA layer selection 对齐，以及 baseline 对比实验。
