...
- 已完成：PAL 核心实现、COCO2014 全量 token extraction、K=512 主模型训练、COCO/Flickr30k Karpathy retrieval、5 个分类数据集 zero-shot、VOC20/Context/ADE20K corrected segmentation rerun、K/`tau_p`/token usage 训练型 sweep、CKA proxy、anchor-overlap 分析。
- 最接近论文的部分：retrieval 与 classification；平均约达到论文指标的 86%~90%。
- 最大差距：dense segmentation，尤其 ADE20K；corrected protocol 后平均 mIoU 从 `5.61` 提升到 `11.33`，但仍低于论文平均 `23.87`。
- 当前主要待办：ADE20K segmentation prompt/name/layer debugging，Context 进一步 dense-token layer / prompt ensemble，K/`tau_p`/token usage checkpoint 的下游指标评测，严格 CKA layer selection 与 baseline rows。
...
最近结果：`Ran 47 tests in 0.183s OK`。
...
当前 sweep 主要记录训练收敛，不等价于论文 ablation table；还需要补 downstream metrics。
...
结论：训练 loss 随 K 增大单调下降，符合 anchor capacity 直觉；但需要 retrieval/classification/segmentation 下游指标才能得出 paper-grade ablation 结论。
...
结论：当前训练 loss 上 `tau_p=0.02` 最低，但低 loss 不一定代表下游最优；需要补 downstream metrics。
...
结论：训练 loss 强支持 token-level CAP 的必要性；但仍需下游指标验证。
...
| ADE20K segmentation | corrected mIoU `2.19` | `13.80` | 15.87%，最大短板 |
| K sweep | K 越大 train loss 越低，K512 `0.283415` | ablation table 需下游指标 | 训练型 sweep 完成 |
| `tau_p` sweep | `tau_p=0.02` train loss `0.252805` 最低 | ablation table 需下游指标 | 训练型 sweep 完成 |
| Token usage | CAP train loss `0.283415` 优于 mean/global | ablation table 需下游指标 | 支持 CAP 方向 |
...
3. **Ablation 下游指标补齐**：对 K、`tau_p`、token usage checkpoints 跑 retrieval/classification/segmentation，不再只用 train loss。
